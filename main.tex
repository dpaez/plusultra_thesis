...
\ctparttext{Ahora es común estar en contacto con algún dispositivo con alta capacidad de computo, mas aun, estos dispositivos pueden ser móviles (\ie smartphones, tablets) y contar con una gran variedad de sensores y actuadores. Estos artefactos tienen la capacidad de aumentar las formas de interacción entre el usuario y el software. Por ejemplo, podria accederse a los motores de vibración y utilizarlo para brindar feedback que estimule el sentido del tacto, junto a una señal auditiva, afectando de esta forma, a mas de un sentido. 
...
%El objetivo de este capitulo es acercar al lector al panorama actual del soporte para el desarrollo de aplicaciones multimodales. 

\part{Introducción} % Primera parte

\include{Chapters/Chapter01_EstadoDelArte} % Chapter 1
...
Al proponer una solución novedosa en determinados aspectos, fue necesario primero buscar algunos puntos de comparación en el desarrollo de plataformas multimodales. A continuación se analizan y comparan la arquitectura de plusultra con los trabajo de Dumas y la W3C.} 

\part{Aspectos Técnicos} % Segunda Parte

\include{Chapters/Chapter02_Infra} % Chapter 2

\cleardoublepage % Empty page before the start of the next part

\include{Chapters/Chapter03_Enlace} % Chapter 3
...
\ctparttext{En esta parte se verá como funcionan los conceptos introducidos en el ámbito de una aplicación web. Se mostrará también el desarrollo de una aplicación web multimodal usando la plataforma para concretar una validación por implementación.}

\part{Desarrollo de una aplicación multimodal}

\include{Chapters/Chapter04_RIA} % Chapter 4

\include{Chapters/Chapter05_DEMO} % Chapter 5

\cleardoublepage % Empty page before the start of the next part

%------------------------------------------------

\ctparttext{El desarrollo de aplicaciones multimodales parece ser el camino a seguir por una parte de la industria. Esto se refleja en el avance en los dispositivos móviles y en diversos proyectos de hardware abierto los cuales ``oxigenan'' las vias de comunicación aun mas. Profundizar y mantener en orbita estos conceptos se vuelve entonces una estrategia adecuada.} 

\part{Conclusiones y trabajo futuro}

\include{Chapters/Chapter06_END}
...
\include{Chapters/Chapter0A_Haptica} % Modalidad háptica
\include{Chapters/Chapter0B_Aero} % Modalidad gestual
...
\cleardoublepage\include{FrontBackMatter/ListOfFigures} % Bibliography
